...
| `nats_idle_heartbeat <seconds>` | `nats_idle_heartbeat` | `10` | Integer in `(0, 15)`; `0` uses default; `-1` disables | Server-side `IdleHeartbeat` interval on the ephemeral JetStream push consumer. When set, the JetStream server emits periodic status messages during quiet periods so nats.go can detect a wedged push path even if the upstream stream is idle — without it, a server-side reap (InactiveThreshold trip during a network blip) or a lost leafnode route leaves the SSE handler attached to a zombie consumer until the client reconnects for other reasons. Default-on at 10s. `-1` is the explicit operator-disable sentinel. Validation enforces the upper bound `< InactiveThreshold/2` (currently 15s) so two missed heartbeats are detectable before the server reaps the consumer. Currently surfaces as `nuts_nats_async_errors_total{kind="consumer_sequence_mismatch"}` (M9 Batch A); milestone M9 Batch B will convert the signal into a clean SSE disconnect with `disconnect_reason=consumer_invalidated`. |
...
| `max_connections <count>` | `max_connections` | `0` | Integer `>= 0` | `0` disables the cap. Rejected clients receive `429` (Too Many Requests, RFC 6585) and `Retry-After: 5`. |
