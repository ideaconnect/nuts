...
- `Handler` struct — all configuration fields (NATS URL, auth, topics, tuning, TLS, CORS, connection caps, health path, client buffer size) plus runtime state (connection, JetStream context, logger, mutex, live-connection counter, explicit-directive flags).
...
- `Provision()` — applies defaults (heartbeat, reconnect, max-event-size, client buffer, health path, allowed headers/methods), calls `connectNATS()`, creates a JetStream context, verifies the configured stream exists, and rolls back on failure.
...
Prometheus counters and gauges registered via `promauto`. Exposes `nuts_active_connections`, `nuts_messages_delivered_total`, `nuts_messages_dropped_total`, `nuts_slow_clients_total`, `nuts_replay_requests_total`, `nuts_replay_fallbacks_total`, `nuts_subscription_errors_total`, and `nuts_connections_rejected_total{reason}` (incremented when `max_connections` rejects a request).
